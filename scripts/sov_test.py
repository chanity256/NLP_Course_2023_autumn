...
if __name__ == "__main__":
    input_file = "/Users/tootough/PycharmProjects/PubtatorProject/sov_input_4.txt"  # 输入文件的路径
    output_file = "/Users/tootough/PycharmProjects/PubtatorProject/output_sov_test_4.txt"  # 输出文件的路径
    ...
    # 在写入文件之前，确保每个元组都包含三个元素
    with open(output_file, 'w', encoding='utf-8') as file:
        for svo in svos:
            if len(svo) >= 3:
                file.write(f"{svo[0]}^${svo[1]}^${svo[2]}\n")

            else:
                print(f"Skipping incomplete tuple: {svo}")
